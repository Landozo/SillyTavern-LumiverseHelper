...
function appendDominantTag(content, tag) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (boldMatch) {
            ...
            // e.g., **Trait Name** -> **Trait Name (My STRONGEST Trait)**
        ...
        if (headerMatch) {
            ...
            // e.g., ## Personality Name -> ## Personality Name (My STRONGEST Personality)
            lines[i] = lines[i].replace(
                /^(\s*)(#{1,6})\s+(.+)$/,
                `$1$2 $3 ${tag}`
            );
        ...
        const contents = selection.map(sel => {
            ...
            if (type === 'behavior' && settings.dominantBehavior) {
                if (settings.dominantBehavior.packName === sel.packName &&
                    ...
                    content = appendDominantTag(content, "(My STRONGEST Trait)");
                }
                ...
                if (settings.dominantPersonality.packName === sel.packName &&
                    ...
                    content = appendDominantTag(content, "(My STRONGEST Personality)");
                }
            }
